...
def find_med_hrs(
    ...
    if extremes_type == 'max':
        ...
        subset = raw_data.resample(time='1D').min()
    else:
    ...
    # Re-order `med_hrs` to have the same location order asd `raw_data` input
...
def retrieve_data(locs, latlons, files, label):
    ...
    def try_load():
        ...
        data = xr.open_mfdataset(files, concat_dim="location", combine="nested").to_array().squeeze("variable")
...
def extract_event_windows(timeseries: xr.DataArray, df: pd.DataFrame, t: int) -> xr.DataArray:
    ...
    if len(event_times) == 0:
        print(f"No valid times found for location={location}, simulation={simulation}, one_in_x={one_in_x}; returning an empty DataArray.")
        n_hours = (t * 2 + 1) * 24
        dummy = xr.full_like(timeseries.isel(time=slice(0, n_hours)), np.nan)
        dummy = dummy.assign_coords(time=np.arange(n_hours)).expand_dims(window=[0]).median(dim='window')
        return dummy
...
def generate_data_to_insert(
    ...
    total_da = total_da.drop_vars('hour_of_year') # Dropping `hour_of_year` because of merge conflict
    to_insert = (
        total_da.squeeze()
...
def create_modified_8760(ds, one_in_x_vals, t, extremes_type='max', custom_times=None, custom_8760=None):
    """
    ...
    daily_ds = combine_ds(make_clean_daily(ds, extremes_type=extremes_type), one_in_x_vals)
    ...
    if isinstance(custom_times, xr.DataArray):
        insert_times = xr.where(~xr.ufuncs.isnan(custom_times), custom_times, find_med_hrs(ds, one_in_x_vals, extremes_type).median(dim="simulation"))
    else:
        insert_times = find_med_hrs(ds, one_in_x_vals, extremes_type=extremes_type).median(dim="simulation")
...
def create_empty_da(da: xr.DataArray, keep_dims: list, fill_value=np.nan) -> xr.DataArray:
    """
...
def set_custom_times(da: xr.DataArray, updates) -> xr.DataArray:
    ...
    if np.isscalar(updates):
        ...
        raise ValueError("`updates` must be either a scalar or a list of coordinate-value dicts.")
